...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embedding_layer = tf.constant(self.pretrained_embeddings)

        embeddings = tf.reshape(tensor=tf.nn.embedding_lookup(embedding_layer, self.input_placeholder),
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.Variable(xavier_initializer((self.config.hidden_size,)))
        U = tf.Variable(xavier_initializer((self.config.hidden_size, self.config.n_classes)))
        b2 = tf.Variable(xavier_initializer((self.config.n_classes,)))

...
class RNNCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope) as scope:
            ...
            try:
                W_x = tf.get_variable("W_x", initializer=tf.contrib.layers.xavier_initializer(), shape=(self.input_size, self._state_size))
                W_h = tf.get_variable("W_h", initializer=tf.contrib.layers.xavier_initializer(), shape=(self._state_size, self._state_size))
                b = tf.get_variable("b", initializer=tf.contrib.layers.xavier_initializer(), shape=(self._state_size,))
            except ValueError:
